...
impl<A: TlbOps> MayNeedFlush<A> {
    fn check_page_size_and_flush_when_needed(&self, page_size: PageSize) {
        let need_global_flush = match page_size {
    ...
    pub fn flush_tlb_global_sync_page(self, vaddr: VirtAddr, page_size: PageSize) {
        self.check_page_size_and_flush_when_needed(page_size);
    ...
    pub fn flush_address_percpu(self, vaddr: VirtAddr) {
        self.check_page_size_and_flush_when_needed(PageSize::Regular);
