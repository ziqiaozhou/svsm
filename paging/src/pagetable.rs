...
impl<A: ArchPagingMeta> PTEntry<A> {
    ...
    fn set_unrestricted(&mut self, addr: PhysAddr, flags: A::PTFlags) {
        let addr = addr.bits();
...
impl<A: ArchPagingMeta, P: PagingHandler> PTPage<A, P> {
    ...
    fn alloc<'a>() -> Result<(&'a mut PTPage<A, P>, PhysAddr), PagingError> {
...
pub struct InactivePageTable<A: ArchPagingMeta, P: PagingHandler, L: PagingLevel> {
    root: PTPage<A, P>,
...
impl<A: ArchPagingMeta, P: PagingHandler, L: PagingLevel> InactivePageTable<A, P, L> {
    ...
    /// # Arguments
    ///
    /// * `idx` - The index of the entry to populate.
    /// * `next_table` - The physical address of the next page table to populate with.
    /// # Returns
    ///
    ...
    pub fn populate(
        &mut self,
        idx: usize,
        next_table: ActivePageTableView<'_, A, P, L::Next>,
    ) -> bool
    where
        L: NextLevel,
        A: TlbOps,
    {
        self.set_entry(idx, next_table.root_pa(), A::PTFlags::parent_flags())
    }
    ...
    pub(crate) fn set_entry(&mut self, idx: usize, addr: PhysAddr, flags: A::PTFlags) -> bool {
...
impl<A: ArchPagingMeta, P: PagingHandler + SelfMap> InactivePageTable<A, P, PagingLevel3> {
...
impl<A: ArchPagingMeta, P: PagingHandler, L: PagingLevel> InactivePageTable<A, P, L> {
    ...
    pub(crate) fn do_set_shared_4k(
        ...
        if let Mapping {
            ...
            entry,
        } = Self::do_try_split_4k_for(mapping, vaddr)?
        {
