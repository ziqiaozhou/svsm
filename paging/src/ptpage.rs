...
impl<A: ArchPagingMeta, P: PagingHandler> PTPage<A, P> {
    ...
    /// # Safety
    /// The caller must ensure that the entry is valid and points to an uninstalled page table.
    unsafe fn from_entry(entry: &PTEntry<A>) -> Option<&'_ Self> {
        // SAFETY: The caller guarantees that the entry is valid and points to an uninstalled page table.
    ...
    /// `*mut` points to the entry at `idx` within a PT page.
...
impl<A: ArchPagingMeta, P: PagingHandler + SelfMap> PTPage<A, P> {
    ...
    pub fn init_self_map(&mut self, root_pa: PhysAddr) {
        ...
        self[P::SELFMAP_IDX].set(A::make_private_address(root_pa), flags);
    }
}
...
impl<A: ArchPagingMeta, P: PagingHandler> PTPage<A, P> {
    ...
    pub(crate) fn do_unmap(mapping: Mapping<'_, A>) -> Option<PageLevel> {
        let entry = mapping.entry;
        match mapping.level {
            PageLevel::Level0 if entry.present() => {
                entry.clear();
                Some(mapping.level)
            }
            PageLevel::Level1 | PageLevel::Level2 if entry.present() && entry.huge() => {
                entry.clear();
                Some(mapping.level)
            }
            _ => {
