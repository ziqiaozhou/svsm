...
impl PerCpu {
    ...
    pub fn init_page_table(&self, pgtable: PageBox<PageTable>) -> Result<(), SvsmError> {
        ...
        // SAFETY: this page table is stored as the active per-CPU page table
        // and is loaded into CR3 during CPU setup; it lives for the lifetime
        // of this (static) PerCpu.
        self.set_pgtable(unsafe { ActivePageTable::new(pgtable) });
