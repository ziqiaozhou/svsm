...
pub use paging::pagetable::{
    ArchPagingMeta, ENTRY_COUNT, GenericPageTableFlags, PagingError, PagingHandler, SelfMap,
};
pub use paging::x86_64::PTEntryFlags;

/// Represents x86 paging mode determined from control registers.
...
/// Set address as private via the architecture-specific encryption mask.
...
fn has_reserved_bits(entry: &PTEntry, pm: PagingMode, level: usize) -> bool {
    ...
    entry.raw() & reserved_mask as usize != 0
}
...
type SvsmPageTable = paging::GenericPageTable<SvsmPaging, SvsmPaging>;

/// Kernel page table newtype wrapping the generic paging crate GenericPageTable.
///
/// Using a newtype instead of a type alias allows adding kernel-specific
/// inherent methods (e.g., `load`, `cr3_value`, `map_region_*`) without
/// requiring callers to import an extension trait.
#[repr(C)]
...
impl PageTable {
    /// Compute the page table index for a given virtual address at level `L`.
    #[inline]
    pub fn index<const L: usize>(vaddr: VirtAddr) -> usize {
        SvsmPageTable::index::<L>(vaddr)
    }

    /// Resolve a virtual address to its [`PageFrame`] via the self-map.
    #[inline]
    ...
    pub unsafe fn load(&self) {
        // SAFETY: Caller guarantees a safe execution state for the new page table.
        unsafe {
    ...
    pub fn allocate_new() -> Result<PageBox<Self>, SvsmError> {
        ...
        let entry = &mut pgtable.root_mut()[PGTABLE_LVL3_IDX_PTE_SELFMAP];
        let flags = PTEntryFlags::PRESENT
            | PTEntryFlags::WRITABLE
            | PTEntryFlags::ACCESSED
            | PTEntryFlags::DIRTY
            | PTEntryFlags::NX;
        entry.set(SvsmPaging::make_private_address(paddr), flags);

        Ok(pgtable)
    }

    /// Clone the shared part of the page table; excluding the private parts.
    ///
    /// # Errors
    /// Returns [`SvsmError`] if the page cannot be allocated.
    ...
    pub fn split_4k(mapping: Mapping<'_>) -> Result<(), SvsmError> {
        match mapping {
            ...
            Mapping::Level1(entry) => {
                SvsmPageTable::do_split_4k(entry)?;
                Ok(())
            }
    ...
    /// Unmaps the virtual memory region.
...
impl RawPageTablePart {
    ...
    /// Walks the page table at level 2 to find the mapping for a given
    /// virtual address.
    fn walk_addr(&mut self, vaddr: VirtAddr) -> Mapping<'_> {
        SvsmPageTable::walk_addr_lvl2(&mut self.page, vaddr)
    }
    ...
    fn alloc_pte_4k(&mut self, vaddr: VirtAddr) -> Mapping<'_> {
        ...
        match m {
            ...
            Mapping::Level1(entry) => {
                SvsmPageTable::alloc_pte_lvl1(entry, vaddr, PageSize::Regular)
            }
            Mapping::Level2(entry) => {
                SvsmPageTable::alloc_pte_lvl2(entry, vaddr, PageSize::Regular)
            }
    ...
    fn alloc_pte_2m(&mut self, vaddr: VirtAddr) -> Mapping<'_> {
        ...
        match m {
            ...
            Mapping::Level2(entry) => SvsmPageTable::alloc_pte_lvl2(entry, vaddr, PageSize::Huge),
            Mapping::Level3(entry) => SvsmPageTable::alloc_pte_lvl3(entry, vaddr, PageSize::Huge),
        }
    }
    ...
    fn map_4k(
        ...
        let addr = if !shared {
            ...
            SvsmPaging::make_shared_address(paddr)
        };
...
impl PTWalkAttr {
    ...
    pub fn check_access_rights(
        ...
        if has_reserved_bits(&entry, self.pm, level) {
