...
* Specifications for dependencies (external crates): If functions, structs, or
  ...
  specifications in `verification/external_specs/`.
* Performance: Store expensive and reusable proofs in `verification/core_proofs/`
  if not provided by `vstd`. The `svsm/kernel/build.rs` sets `rlimit=1`, while
  `verification/core_proofs/build.rs` sets `rlimit=4`, helping developers
